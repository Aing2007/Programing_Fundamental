...
int main(void)
{
    ...
    struct Artists
    {
        char name[5][50]; // ให้เก็บชื่อเป็น array 2 มิติ
        struct album_data album[10];
        int num_album;
    };

    struct Artists artist[5]; // <== แก้ให้ artist เป็น Array
    int i;

    strcpy(artist[0].name, "The Corrs"); // แก้การ Assign ค่า โดยใช้ index array
    ...
    for (int k = 0; k < 5; k++) // <==การแสดงผล ให้ใช้ Nasted Loop ในการเรียก index
    {
        printf("\n-- Artist: %s --\n", artist[k].name);
